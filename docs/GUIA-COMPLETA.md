# Guia completa de Umine Voice

Todo el detalle que el `README.md` resume: como funciona por dentro, que expone la API, como
vuelve el resultado de una llamada, como salir del modo simulacion y que falta por hacer.

Si vas a escribir codigo, leer ademas `CLAUDE.md`: ahi estan las convenciones y las reglas de
seguridad que no se negocian.

## Indice

1. [Como funciona](#como-funciona)
2. [El Semaforo: tres secciones, dos motivos de llamada](#el-semaforo-tres-secciones-dos-motivos-de-llamada)
3. [Las cuatro vistas del micrositio](#las-cuatro-vistas-del-micrositio)
4. [API de operacion](#api-de-operacion)
5. [Como vuelve el resultado de una llamada](#como-vuelve-el-resultado-de-una-llamada)
6. [Llamadas reales](#llamadas-reales-salir-del-modo-simulacion)
7. [Configuracion](#configuracion)
8. [Calidad y testing](#calidad-y-testing)
...
```
  ...
  course-lookup ──► urgency-classifier  (nivel del Semaforo: NORMAL/ALERTA/CRITICO)
        ...
        v
  [una persona hace click]  Disparador  o  guardado del Tablero Mock
        |
        v
  ...
  FOLLOWUP READY ──(escritura condicional)──► DIALING
        |
        v
  ElevenLabs origina la llamada por Twilio, y conversa con la persona
        |
        v
...
- **El nivel del Semaforo y la decision de llamar son cosas distintas.**
  `urgency-classifier.ts` es un espejo 1:1 del Semaforo real y **no se configura**. Si se
  pudiera, el tablero dejaria de mostrar lo mismo que el original y perderia todo su valor como
  referencia. `call-rules.ts` decide si esa situacion amerita una llamada, y eso si es
  configurable. Por defecto coinciden: llama en CRITICO.
- **Un solo modulo decide que pasa con un resultado**: `services/call-result-recorder.ts`. Entre
  por webhook, por sync o por reconciliacion con Twilio, la logica es la misma. Una regla nueva
  va ahi, nunca en un handler.

## El Semaforo: tres secciones, dos motivos de llamada

La misma OC se evalua por **tres criterios independientes a la vez** y puede caer en dos
secciones o en ninguna. Las tres escalas no se mezclan: un CRITICO de conexion no es un CRITICO
de DJ.

| Seccion                | Que mira                                                                      | Escala                                                                  | ¿Llama? |
| ---------------------- | ----------------------------------------------------------------------------- | ----------------------------------------------------------------------- | ------- |
| **A. Riesgo Conexion** | Curso en ejecucion, sin terminar, con porcentaje de conexion incompleto       | Semana del curso contra su umbral (S2 <55%, S3 <80%, S4 <90% = CRITICO) | **Si**  |
...
| **C. Rectificacion**   | OC en ESPERA o RECTIFICACION hace mas de 3 dias                               | Dias esperando al OTIC                                                  | **No**  |
...
otro lado**. Hay un test en `web/` que lo verifica.

| Modulo                     | Que resuelve                                       |
| -------------------------- | -------------------------------------------------- |
| `urgency-classifier.ts`    | Las tres escalas de criticidad                     |
| `semaforo-sections.ts`     | Los filtros de entrada de cada seccion             |
| `order-status-promoter.ts` | Promocion de estado por fechas y agregacion por OC |
| `course-lookup.ts`         | Los compone                                        |

Los cuatro son espejo del repo `micrositio-operaciones-tablero-sence`: un cambio de umbral alla
exige el cambio equivalente aca. Contrato del API, paginacion y limitaciones en
`docs/SEMAFORO_INTEGRACION.md`.
...
| **Disparador**       | Elegir una OC llamable mas un telefono, ver el modal de confirmacion y disparar UNA llamada                                                                                                                                       |

El Disparador deja **escribir el telefono a mano** a proposito. Eso no debilita nada: el
guardrail vive en el backend (`services/guardrails.ts`), asi que un numero no autorizado responde
403 igual. Lo que hace el front es avisar el motivo antes. **No muevas ese chequeo al front.**
...
| ------ | ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GET    | `/api/health`           | Kill switch, `DRY_RUN`, `MOCK_PROVIDERS`, modo del tablero, cuota usada y restante, ventana horaria, allowlist, estado del webhook y del sync                                   |
| GET    | `/api/tablero`          | Las OCs candidatas a llamada, con urgencia, seccion, semana, porcentaje de conexion, DJ pendientes, contacto, telefono enmascarado y las `dynamic_variables` exactas del agente |
...
| POST   | `/api/calls/sync`       | Trae los resultados desde la API de ElevenLabs y Twilio. Solo `GET` contra los proveedores: **no origina llamadas**                                                             |
| GET    | `/api/calls`            | Seguimientos con su ultimo resultado clasificado                                                                                                                                |
| GET    | `/api/calls/:id`        | Detalle del seguimiento y todas sus llamadas con transcripcion                                                                                                                  |
...
| PUT    | `/api/tablero/mock/auto-call`                     | Enciende o apaga el disparo automatico **por seccion**. Exige `seccion` y rechaza `C_RECTIFICACION`                         |
...
Operacion, validacion y limites del historial: `docs/architecture/CALL-HISTORY.md`.
...
Body: `clientId`, `orderNumber` y `phone` son obligatorios; `seccion` es opcional
...
| 400    | Falta `Idempotency-Key`, falta `clientId`, `orderNumber` o `phone`, o `seccion` no soportada    |
...
### 1. PULL con `npm run calls:sync`, el que funciona siempre
...
- **Clasificar bien una llamada que nadie atendio.** El `status` de ElevenLabs nunca dice
  ...
  cerraba como contactada. Si Twilio dice `no-answer`, `busy`, `failed` o `canceled`, **ese
  resultado manda**. Si dice `completed`, decide la conversacion como siempre: Twilio no sabe de
  que se hablo.
- **Destrabar los seguimientos colgados en `DIALING`.** Las llamadas que no dejan ninguna
  conversacion no aparecen en el listado de ElevenLabs. Una segunda pasada revisa cada FOLLOWUP
  en `DIALING` contra el `call_sid` que guardo el dispatcher.
...
ElevenLabs tampoco se cierra, porque el resultado bueno esta por llegar.
...
`CONVERSATION#<id> META` que escribe el dispatcher, y si no esta, el `followup_id` que viaja
dentro de la conversacion en ElevenLabs. Lo que no puede atribuir con certeza **no lo toca**: lo
reporta como no atribuible, porque una atribucion equivocada ensucia el registro de otra llamada.
Para eso existe la atribucion manual (`--conversation-id` mas `--followup-id`), donde decide un
humano.

> Las conversaciones que no origino este proyecto, por ejemplo las pruebas hechas desde el panel
> de ElevenLabs, siempre apareceran como no atribuibles. Es correcto: no traen ninguna referencia
> a una OC.

### 2. PUSH con el webhook post-call, el camino de produccion
...
```bash
npm run webhook:connect      # tunnel, registro HMAC y asignacion al agente (necesita webhooks_write)
# o, a mano:
npm run tunnel:up            # solo el tunnel; escribe PUBLIC_BASE_URL en .env
...
- Activalo como **post-call webhook** en **Agents Platform, Settings**. Crearlo no alcanza: si no
  queda asignado ahi, ElevenLabs no le entrega nada. Un agente puede tener su propio override, que
  gana sobre la configuracion del workspace.

Verificalo **sin gastar un minuto de llamada**:

```bash
npm run webhook:selftest     # firma valida devuelve 200, firma invalida devuelve 401
...
- **Una API key** (Settings, API Keys) con acceso a Agents Platform, en `ELEVENLABS_API_KEY`.
  Para el historial y el webhook necesita ademas leer conversaciones y permiso `webhooks_write`.

Importante: **la llamada la origina ElevenLabs, no este backend.** Nosotros solo hacemos
`POST /v1/convai/twilio/outbound-call` (integracion nativa, ADR-003). Las credenciales de Twilio
de nuestro `.env` se usan para validar la firma del webhook de status y, sobre todo, para que
`calls:sync` pueda preguntar como termino cada llamada.

El contrato conversacional, es decir las variables que el agente espera recibir, vive en
`src/services/agent-variables.ts` y se verifica contra el agente remoto, sin llamar a nadie:
...
importados, confirma que los IDs del `.env` existen de verdad, chequea las credenciales de Twilio
y avisa de configuraciones riesgosas, por ejemplo una ventana horaria de 24 horas o una cuenta de
Twilio en Trial. Resolve todos los `FALTA` antes de seguir.
...
los numeros de `TELEFONOS_ETAPA_PRUEBAS` que ademas esten en `ALLOWLIST_NUMBERS`, con
`DAILY_QUOTA` llamadas por dia, dentro de la ventana horaria, y `KILL_SWITCH=true` corta todo al
instante.
...
- **Regulacion chilena (UV-027)**: Twilio exige numeros `+56 600` o `+56 809` con KYC local para
...
- **`CALL_RECORDING_ENABLED`**: default `false` (UV-026, decision de negocio abierta). Se manda
  explicito en cada request, no se hereda de la configuracion del agente.
- **Un seguimiento colgado en `DIALING` casi nunca es un bug**: es que el resultado no volvio.
  Corre `npm run calls:sync`. Si aparece como no atribuible, esa conversacion no la origino este
  proyecto. Si aparece como `sin_datos`, el seguimiento se origino antes de que se guardara el
  `call_sid`, o el SID es de otra cuenta: hay que mirarlo en el log de Twilio y atribuirlo a mano.
- **Que no contestaron lo resuelve Twilio, no ElevenLabs (UV-053)**: el `data.status` real solo
  toma `initiated`, `in-progress`, `processing`, `done` y `failed`. **Sin `TWILIO_ACCOUNT_SID` ni
  `TWILIO_AUTH_TOKEN`**, o con credenciales de otra cuenta, una llamada sin atender sigue cayendo
  como `contacted` y luego `CERRADO`: el webhook por si solo no alcanza para distinguirla.
...
Todas las variables estan en `.env.example`, que lleva **solo nombres, nunca valores reales**. Las
que mas importan:
...
| `BUSINESS_HOURS_START/_END`  | `09:00` y `19:00` | Ventana horaria, en `TIMEZONE` (`America/Santiago`)        |
| `TABLERO_API_MODE`           | `fixture`         | `fixture` usa el Tablero Mock editable; `http` el real     |
...
npm test               # 402 tests en 27 archivos (unit e integracion contra dynalite real)

npm run web:build      # tsc --noEmit mas vite build del micrositio
npm run web:test       # 80 tests del front (vitest y jsdom)
```
...
- **Unitario** (`test/unit/`): logica pura, sin red ni DynamoDB. Clasificador de urgencia,
  secciones, reglas de llamada, guardrails, idempotencia, validadores de firma y taxonomia de
  resultados.
- **Integracion** (`test/integration/`): `dynalite` real, no mocks del SDK, para verificar
  escrituras condicionales, GSIs, los handlers completos y la API via `fastify.inject()`. Cada
  test de rechazo verifica ademas que el proveedor **no** se invoco y que la cuota no se movio.
- **Front** (`web/test/`): vitest, jsdom y testing library. El test central es que el micrositio
  no dispara sin pasar por el modal de confirmacion, y que no reimplementa la logica del Semaforo.

**Ningun test llama de verdad a Twilio ni a ElevenLabs.** Siempre `MockElevenLabsClient` o `fetch`
...
**Modo simulacion, operable de punta a punta.** El Semaforo completo con sus tres secciones, el
Tablero Mock editable, el disparo manual con todos los guardrails, el registro de llamadas por los
tres caminos y el historial del agente estan implementados y con tests. `infra/` (CDK) esta
construido pero **no desplegado**.

Pendientes que hay que tener en cuenta. El backlog completo esta en `docs/spec.csv`, con 17
tickets abiertos:

| Ticket       | Que falta                                                                                                | Impacto                                                                          |
| ------------ | -------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- |
| **UV-042**   | La `events.Rule` de cron en `infra/` sigue declarada                                                     | **Bloqueante de despliegue**: contradice que ninguna llamada se dispare sola     |
| UV-027       | Twilio exige numeros `+56 600` o `+56 809` con KYC local en Chile                                        | Bloqueante externo antes de llamar a un cliente real                             |
...
| UV-023 a 028 | Decisiones de negocio abiertas: a quien llamar, fuente del telefono, disclosure de IA, grabacion y cuota | Modeladas como **configuracion**, nunca hardcodeadas                             |
